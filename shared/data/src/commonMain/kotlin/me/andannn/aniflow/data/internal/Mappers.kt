...
import me.andannn.aniflow.data.model.define.deserializeStringKeyEnum
...
internal fun MediaEntity.toDomain() =
    MediaModel(
        id = id,
        type = mediaType?.let { StringKeyEnum.deserializeStringKeyEnum(it) },
        title =
            Title(
        ...
        description = description,
        source = source?.let { StringKeyEnum.deserializeStringKeyEnum(it) },
        status = status?.let { StringKeyEnum.deserializeStringKeyEnum(it) },
        format = format?.let { StringKeyEnum.deserializeStringKeyEnum(it) },
        bannerImage = bannerImage,
        averageScore = averageScore?.toInt(),
        favourites = favourites?.toInt(),
        season = season?.let { StringKeyEnum.deserializeStringKeyEnum(it) },
...
internal fun MediaListEntity.toDomain() =
    MediaListModel(
        id = mediaListId,
        status = listStatus?.let { StringKeyEnum.deserializeStringKeyEnum(it) },
