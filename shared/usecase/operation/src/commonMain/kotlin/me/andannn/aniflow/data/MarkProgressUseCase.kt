    ...
    if (error == null) {
        ...
        val result =
            snackBarMessageHandler.showSnackBarMessageSuspend(
                if (isCompleted) {
                    SnackBarMessageDetail.MediaMarkCompleted(title, newProgress)
                } else {
                    SnackBarMessageDetail.MediaMarkWatched(title, newProgress)
                },
            )
...
private suspend fun onMarkDropped(
    ...
    if (error == null) {
        ...
        val result =
            snackBarMessageHandler.showSnackBarMessageSuspend(
                SnackBarMessageDetail.MediaMarkDropped(title),
            )
        if (result == SharedSnackbarResult.ActionPerformed) {
            // Undo action performed.
            Napier.d(tag = TAG) { "onMarkProgress: Undo performed E" }
